...
//	OrthoWidth = 1024.0f * 2.0f;
//	AspectRatio = 9.0f / 16.0f;
//	//SetupAttachment(SpringArm, USpringArmComponent::SocketName);
//	//SetWorldRotation(FRotator(-90.0, 0.0, 0.0));
//}

void UCustomCameraComponent::OnZoomIn()
{
	SetZoomLevel(ZoomAlpha - 0.1f);
}

void UCustomCameraComponent::OnZoomOut()
{
	SetZoomLevel(ZoomAlpha + 0.1f);
}

void UCustomCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& OutResult)
{
...
void UCustomCameraComponent::UpdateCameraMovement(const APlayerController* InPlayerController)
{
...
void UCustomCameraComponent::MoveForward(float Val)
{
...
void UCustomCameraComponent::MoveRight(float Val)
{
	APawn* OwnerPawn = GetOwnerPawn();
	if (OwnerPawn != NULL)
	{
...
void UCustomCameraComponent::AddNoScrollZone(FBox InCoords)
{
	NoScrollZones.AddUnique(InCoords);
}

void UCustomCameraComponent::ClampCameraLocation(const APlayerController* InPlayerController, FVector& OutCameraLocation)
{
...
void UCustomCameraComponent::UpdateCameraBounds(const APlayerController* InPlayerController)
{
	ULocalPlayer* const LocalPlayer = Cast<ULocalPlayer>(InPlayerController->Player);
...
APlayerController* UCustomCameraComponent::GetPlayerController()
{
...
APawn* UCustomCameraComponent::GetOwnerPawn()
{
	return Cast<APawn>(GetOwner());
}

void UCustomCameraComponent::OnPinchStarted(const FVector2D& AnchorPosition1, const FVector2D& AnchorPosition2, float DownTime)
{
...
//void UCustomCameraComponent::OnPinchUpdate(UStrategyInput* InputHandler, const FVector2D& ScreenPosition1, const FVector2D& ScreenPosition2, float DownTime)
//{
...
void UCustomCameraComponent::SetCameraTarget(const FVector& CameraTarget)
{
...
void UCustomCameraComponent::SetZoomLevel(float NewLevel)
{
	ZoomAlpha = FMath::Clamp(NewLevel, MinZoomLevel, MaxZoomLevel);
}

bool UCustomCameraComponent::OnSwipeStarted(const FVector2D& SwipePosition)
{
	bool bResult = false;
...
bool UCustomCameraComponent::OnSwipeUpdate(const FVector2D& SwipePosition)
{
	bool bResult = false;
	APlayerController* Controller = GetPlayerController();
...
bool UCustomCameraComponent::OnSwipeReleased(const FVector2D& SwipePosition)
{
	bool bResult = false;
...
void UCustomCameraComponent::EndSwipeNow()
{
	StartSwipeCoords.Set(0.0f, 0.0f, 0.0f);
}

bool UCustomCameraComponent::AreCoordsInNoScrollZone(const FVector2D& SwipePosition)
{
	bool bResult = false;
